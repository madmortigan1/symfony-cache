...
if [[ "$ENABLE_XDEBUG" == "1" ]] ; then
  ...
  echo "zend_extension=$(find /usr/local/lib/php/extensions/ -name xdebug.so)" > $XdebugFile
	...
	echo "xdebug.discover_client_host = false" >> $XdebugFile
	echo "xdebug.max_nesting_level = 500" >> $XdebugFile
	echo "xdebug.profiler_append = 0" >> $XdebugFile
	echo "xdebug.mode=debug,develop,trace" >> $XdebugFile
...
# SASS compilation directory
if [ -d "/var/www/html/public/css/sass-cache" ]; then
    # Allow sass compilation
    chmod 777 /var/www/html/public/css/sass-cache; sync;
  else
    echo "Can't find sass-cache directory"
fi
